...
% the code, change the date_for_folder_filenames accordingly, e.g.:
date_for_folder_filenames = '221229';
...
%Ex vivo table with results
exvivo_date_analysis = '230104';
load(fullfile(path_for_data,'ExVivo','Results',[exvivo_date_analysis '_M1M2_SingleMeas'],'ExVivoResults_IrregularBinning_NotNormalised.mat'));
...
% Cylinders
Data = load(fullfile(path_for_data,'InSilico','Directions1500_Cartesian.mat')); % If defined by text file
            ...
            for dt_indx = 1:numel(datatype_indexes) % Go through all desired datatypes.
                ...
                for norm_indx = 1:2 % Without/with normalisation with first echo        
                    for max_indx = 1:numel(max_time) % Maximum time index
                        ...
                        for ti = 1:time_end
                            time_indx_exp(ti) = max(find(sim_time_range <= exp_time_range(ti)));
                        end
                        ...
                        for k = 1:numel(kappa_values)
                            for j = 1:numel(angle_values)
                                ...
                                time_exp = sim_time_range(time_indx_exp)';
...
%% Anisotropic binning and statistics estimation (i.e. wAICc)
% To note, for any "metric" like AIC, AICc or BIC, the "average" is
% obtained in function of the "average" of the corresponding metric across
% all samples per TEmax, dispersion, model and angle.
clc;
...
IrregBinNum = 20; %This number must be the same used for the irregular binning in ex vivo

params_fig.MinEdgeExVivoIrregularBin = reshape(ExVivoTableSummary.MinEdge_AngBin(ExVivoTableSummary.TEmax == 54,:),[IrregBinNum,3]);
params_fig.MaxEdgeExVivoIrregularBin = reshape(ExVivoTableSummary.MaxEdge_AngBin(ExVivoTableSummary.TEmax == 54,:),[IrregBinNum,3]);

%This is saved in two tables: Myelin and non-myelin simulations.
Myelin_InSilicoResults = [];
NoMyelin_InSilicoResults = [];
...
for i = 1:6
    ...
    disp(['Creating table for ' params_fig.datatype ' with g-ratio ' num2str(g_samples(i)) ' and ' params_fig.T2Reference]);
    Aux_NoMyelin_InSilicoResults = AnisotropicBinning_InSilico_fromExVivo(fullfile(folder_fitted_signal,['FittedSignal_' datatypes{datatype_indexes(1)} '_1500cylinders_TimeES_' reg_string '_NNormalised_SNR' num2str(SNR_study)  '_FVF5' suffix_string{i} '.mat']),params_fig);
    params_fig.datatype = 'Mye';
    disp(['Creating table for ' params_fig.datatype ' with g-ratio ' num2str(g_samples(i)) ' and ' params_fig.T2Reference]);
    Aux_Myelin_InSilicoResults = AnisotropicBinning_InSilico_fromExVivo(fullfile(folder_fitted_signal,['FittedSignal_' datatypes{datatype_indexes(2)} '_1500cylinders_TimeES_' reg_string '_NNormalised_SNR' num2str(SNR_study) '_FVF5'  suffix_string{i} '.mat']),params_fig);
    NoMyelin_InSilicoResults = [NoMyelin_InSilicoResults;Aux_NoMyelin_InSilicoResults];
    Myelin_InSilicoResults = [Myelin_InSilicoResults;Aux_Myelin_InSilicoResults];
end

% Save data here.
save(fullfile(path_for_data,'InSilico',['Myelin_NoMyelin_AnisotropicBinarised_InSilicoMR_' date_for_folder_filenames '.mat']),'NoMyelin_InSilicoResults','Myelin_InSilicoResults');
