...
if strfind(color{1},'b')
    h=plot(x,y,'--','Color',ColorRGB_ForPlot(1),'LineWidth',2); % main plot
    if strfind(color{1},'o')
        h=plot(x,y,'o--','Color',ColorRGB_ForPlot(1),'LineWidth',2,'MarkerSize',6);
    end
    color{2} = ColorRGB_ForPlot(1);
    color{3} = ColorRGB_ForPlot(1);
elseif strfind(color{1},'y')
    h=plot(x,y, 'LineStyle','--','Color',ColorRGB_ForPlot(2),'LineWidth',2);
    if strfind(color{1},'s')
        h=plot(x,y,'s--','Color',ColorRGB_ForPlot(2),'LineWidth',2,'MarkerSize',8);
    end
    color{2} = ColorRGB_ForPlot(2);
    color{3} = ColorRGB_ForPlot(2);
elseif strfind(color{1},'r')
    h=plot(x,y, 'LineStyle','--','Color',ColorRGB_ForPlot(3),'LineWidth',2);
    if strfind(color{1},'x')
        h=plot(x,y,'x--','Color',ColorRGB_ForPlot(3),'LineWidth',2,'MarkerSize',8);
    end
    color{2} = ColorRGB_ForPlot(3);
    color{3} = ColorRGB_ForPlot(3);
elseif strfind(color{1},'u')
    h=plot(x,y, 'LineStyle','--','Color',ColorRGB_ForPlot(4),'LineWidth',2);
    if strfind(color{1},'x')
        h=plot(x,y,'x--','Color',ColorRGB_ForPlot(4),'LineWidth',2,'MarkerSize',8);
    end
    color{2} = ColorRGB_ForPlot(4);
    color{3} = ColorRGB_ForPlot(4);  
elseif strfind(color{1},'t')
    h=plot(x,y, 'LineStyle','--','Color',ColorRGB_ForPlot(5),'LineWidth',2);
    if strfind(color{1},'x')
        h=plot(x,y,'x--','Color',ColorRGB_ForPlot(5),'LineWidth',2,'MarkerSize',8);
    end
    color{2} = ColorRGB_ForPlot(5);
    color{3} = ColorRGB_ForPlot(5);  
elseif strfind(color{1},'z')
    h=plot(x,y, 'LineStyle','--','Color',ColorRGB_ForPlot(6),'LineWidth',2);
    if strfind(color{1},'x')
        h=plot(x,y,'x--','Color',ColorRGB_ForPlot(6),'LineWidth',2,'MarkerSize',8);
    end
    color{2} = ColorRGB_ForPlot(6);
    color{3} = ColorRGB_ForPlot(6);  
end
    
allh=nan(length(x), 6); % all errorbar handles
for k=1:length(x)
    if ~isempty(lx) & ~isempty(ly) % both errors are specified
        l1=line([lx(k) ux(k)],[y(k) y(k)],'LineWidth',2);
        hold on;
        l2=line([lx(k) lx(k)],[y(k)-0.1*erry(k) y(k)+0.1*erry(k)],'LineWidth',2);
        l3=line([ux(k) ux(k)],[y(k)-0.1*erry(k) y(k)+0.1*erry(k)],'LineWidth',2);
        l4=line([x(k) x(k)],[ly(k) uy(k)],'LineWidth',2);
        l5=line([x(k)-0.1*errx(k) x(k)+0.1*errx(k)],[ly(k) ly(k)],'LineWidth',2);
        l6=line([x(k)-0.1*errx(k) x(k)+0.1*errx(k)],[uy(k) uy(k)],'LineWidth',2);
        allh(k, :)=[l1, l2, l3, l4, l5, l6];
    elseif isempty(lx) & ~isempty(ly) % x errors are not specified
        l4=line([x(k) x(k)],[ly(k) uy(k)],'LineWidth',2);
        hold on;
        errx=nanmean(abs(diff(x)));
        l5=line([x(k)-0.1*errx x(k)+0.1*errx],[ly(k) ly(k)],'LineWidth',2);
        l6=line([x(k)-0.1*errx x(k)+0.1*errx],[uy(k) uy(k)],'LineWidth',2);
        allh(k, 4:6)=[l4, l5, l6];
    elseif ~isempty(lx) & isempty(ly) % y errors are not specified
        l1=line([lx(k) ux(k)],[y(k) y(k)],'LineWidth',2);
        hold on;
        erry=nanmean(abs(diff(y)));
        l2=line([lx(k) lx(k)],[y(k)-0.1*erry y(k)+0.1*erry],'LineWidth',2);
        l3=line([ux(k) ux(k)],[y(k)-0.1*erry y(k)+0.1*erry],'LineWidth',2);
        allh(k, 1:3)=[l1, l2, l3];
